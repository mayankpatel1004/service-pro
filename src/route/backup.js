...
router.post('/services', checkTokenExists,async (req, res) => {
	...
	if (req.body.action == 'update_status' && ['Y', 'N', 'T'].includes(req.body.status)) {
		let sqlUpdateStatus = ``;
		if (req.body.status == 'T') {
			sqlUpdateStatus = `UPDATE ${DBTABLES.SERVICE} SET deleted_status = 'Y' WHERE service_id IN (${req.body.pk_ids})`;
		} else {
			sqlUpdateStatus = `UPDATE ${DBTABLES.SERVICE} SET display_status = '${req.body.status}' WHERE service_id IN (${req.body.pk_ids})`;
		}
        console.log(sqlUpdateStatus);
		await db.query(sqlUpdateStatus, async (error, results, fields) => {
			res.send({
...
router.get('/service_form', checkTokenExists, async (req, res) => {
	...
	if (req.query.edit_id && req.query.edit_id > 0) {
		edit_id = req.query.edit_id;
		let sqlServiceForm = `SELECT * FROM ${DBTABLES.SERVICE} WHERE service_id = '${edit_id}'`;
		let sqlServiceFormMeta = `SELECT * FROM ${DBTABLES.SERVICE_META} WHERE service_program_id = '${edit_id}'`;
		await db.query(sqlServiceForm, async (error, results, fields) => {
			await db.query(sqlServiceFormMeta, async (error, resultsMeta, fields) => {
				if (results && results.length > 0) {
					arrMetaFields = resultsMeta;
	...
	setTimeout(async () => {
		...
		arrFields.push({
			...
			ph: '',
			req: 'N',
			options: await functions.getClients(),
			cls: 'form-control js-example-basic-single formfields',
		});
		arrFields.push({
			type: 'select',
			...
			ph: '',
			req: 'N',
			options: await functions.displayStatus(),
			cls: 'form-control js-example-basic-single formfields',
		});
...
router.post('/service_form', userImageUpload, async (req, res) => {
	...
	await db.query(sqlSave, async (error, results, fields) => {
		...
		if(metaKeys && metaKeys.length > 0){
			let sqlDeleteData = `DELETE FROM ${DBTABLES.SERVICE_META} WHERE service_program_id = ${saved_id}`;
				...
				for (const [key,item] of Object.entries(metaKeys)){
					if(item != ""){
						if(data[item] == 1){
							...
							let sqlInsert = `INSERT INTO ${DBTABLES.SERVICE_META} SET service_program_id = ${saved_id}, sub_service_id = '${sub_service_id}'`; 
